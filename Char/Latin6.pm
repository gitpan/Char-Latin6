...
######################################################################
#
# Char::Latin6 - Source code filter to escape Latin-6 script
#
# http://search.cpan.org/dist/Char-Char::Latin6/
#
...
BEGIN { CORE::require Char::Elatin6; }
...
my $function_ord;         # ord()   to ord()   or Char::Latin6::ord()
my $function_ord_;        # ord     to ord     or Char::Latin6::ord_
my $function_reverse;     # reverse to reverse or Char::Latin6::reverse
my $function_getc;        # getc    to getc    or Char::Latin6::getc
...
END
    ...
    print Char::Latin6::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Elatin6::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Latin6::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Elatin6::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Latin6::escape_script {
    ...
    # read Latin-6 script
    my $fh = gensym();
    Char::Elatin6::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Elatin6(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Elatin6 %s;\n", $Char::Latin6::VERSION); # require run-time routines version

        # use Char::Latin6 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Latin6 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Latin6::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Latin6.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/Latin6.pm $version, but @{[__FILE__]} is version $Char::Latin6::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Elatin6.pm $version, but \\\$Char::Elatin6::VERSION is \$Char::Elatin6::VERSION" if \$Char::Elatin6::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Latin6::VERSION) {
                    die "Script $0 required Char/Latin6.pm $version, but @{[__FILE__]} is only version $Char::Latin6::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Latin6::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Latin6::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Latin6::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Latin6::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Char::Elatin6
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Latin6::re_t=%s; while($Char::Latin6::re_t =~ %s){%s local $^W=0; local $Char::Latin6::re_r=%s; %s$Char::Latin6::re_t="$`$Char::Latin6::re_r$'"; pos($Char::Latin6::re_t)=length "$`$Char::Latin6::re_r"; } return $Char::Latin6::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Latin6::re_r=eval $Char::Latin6::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Latin6::re_r=%s; %s"%s$Char::Latin6::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Latin6::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Latin6::re_r=%s; %s%s="$`$Char::Latin6::re_r$'"; pos(%s)=length "$`$Char::Latin6::re_r"; $Char::Latin6::re_n++} return %s$Char::Latin6::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Latin6::re_r=%s; %s%s="%s$Char::Latin6::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Latin6::re_r=%s; %s%s="%s$Char::Latin6::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Latin6::re_r=eval $Char::Latin6::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Latin6.pm and Char/Elatin6.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Latin6;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Latin6;
  use Char::Latin6 ver.sion;             --- require minimum version
  use Char::Latin6 ver.sion.0;           --- expects version (match or die)

  # "no Char::Latin6;" not supported

  or

  $ perl Char/Latin6.pm Latin-6_script.pl > Escaped_script.pl.e
...
Char::Latin6 software is "middleware" between perl interpreter and your Perl script
written in Latin-6.
...
By "use Char::Latin6;", it automatically interpret your script as Latin-6. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/Latin6.pm, Char/Elatin6.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Elatin6.pm
works well.
...
=head1 Software Composition
   ...
   Char/Latin6.pm               --- source code filter to escape Latin-6
   Char/Elatin6.pm              --- run-time routines for Char/Latin6.pm
...
You need write 'use Char::Latin6;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::Latin6;
  ---------------------------------

=head1 Calling 'Char::Elatin6::ignorecase()' (Char/Latin6.pm provides)

Char/Latin6.pm applies calling 'Char::Elatin6::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Elatin6::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Elatin6.pm provides)
...
Definitions in Char/Elatin6.pm.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Latin6.pm provides)

Char/Latin6.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Latin6.pm and Char/Elatin6.pm provide)

Insert 'Char::Elatin6::' at head of function name. Char/Elatin6.pm provides your script Char::Elatin6::*
functions.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Elatin6::lc        Character
  lcfirst     Char::Elatin6::lcfirst   Character
  uc          Char::Elatin6::uc        Character
  ucfirst     Char::Elatin6::ucfirst   Character
  fc          Char::Elatin6::fc        Character
  chr         Char::Elatin6::chr       Character
  glob        Char::Elatin6::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Functions (Char/Latin6.pm provides)

Char/Latin6.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Elatin6.pm provides)

Char/Elatin6.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::Latin6' }" at head.
Store the standard module modified for Char::Latin6 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::Latin6 and change
'use utf8;' to 'use Char::Latin6;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Elatin6.pm provides these
  functions.
...
Char::Latin6 software remains one language and one interpreter.
