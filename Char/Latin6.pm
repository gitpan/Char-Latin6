...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Elatin6::uc($char[$i]) ne Char::Elatin6::lc($char[$i]))) {
            $char[$i] = '[' . Char::Elatin6::uc($char[$i]) . Char::Elatin6::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Latin6/Char::Latin6/JA.pm
...
Definitions in Char/Elatin6.pm.
  ...
  @{Char::Elatin6::eV}             qr{(?:[^\x0C\x0A\x0D])}
