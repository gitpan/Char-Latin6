...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Elatin6::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Elatin6::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Elatin6::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Elatin6::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Elatin6::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Elatin6::uc($_[0]) =~ m{\A $pattern \z}xms };
...
This module is a run-time routines of the Latin6.pm.
Because the Latin6.pm automatically uses this module, you need not use directly.
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Elatin6::split returns
...
=item Chop string
  ...
  The Elatin6::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Elatin6::index($string,$substr,$position);
  $pos = Elatin6::index($string,$substr);
  ...
  the Elatin6::index function returns -1.

=item Reverse index string

  $pos = Elatin6::rindex($string,$substr,$position);
  $pos = Elatin6::rindex($string,$substr);

  Works just like Elatin6::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Elatin6::chr(65) is "A" in either ASCII or Latin-6, and
  Elatin6::chr(0x82a0) is a Latin-6 HIRAGANA LETTER A. For the reverse of Elatin6::chr,
  use Latin6::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Elatin6::binmode(FILEHANDLE, $disciplines);
  Elatin6::binmode(FILEHANDLE);
  Elatin6::binmode($filehandle, $disciplines);
  Elatin6::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Elatin6::open(FILEHANDLE, $mode, $expr);
  $rc = Elatin6::open(FILEHANDLE, $expr);
  $rc = Elatin6::open(FILEHANDLE);
  $rc = Elatin6::open(my $filehandle, $mode, $expr);
  $rc = Elatin6::open(my $filehandle, $expr);
  $rc = Elatin6::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Elatin6::open($fh, ">logfile")     # $fh is autovivified
  ...
  Elatin6::open my $fh, ">logfile" or die ...
  ...
  Elatin6::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Elatin6::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Elatin6::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Elatin6::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Elatin6::open(FH, "<:crlf:encoding(Latin6)", $path) or die "can't open $path: $!";
