...
######################################################################
#
# Latin6 - Source code filter to Latin-6 script
#
...
my $function_ord;         # ord()   to ord()   or Latin6::ord()
my $function_ord_;        # ord     to ord     or Latin6::ord_
my $function_reverse;     # reverse to reverse or Latin6::reverse
...
sub Latin6::escape_script {
    ...
    if (m/^ use Elatin6(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $e_script .= sprintf("use Elatin6 %s;\n%s", $Elatin6::VERSION, $use_re_eval); # require run-time routines version

        # use Latin6 version qw(ord reverse);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        $function_reverse = 'reverse';
        if (s/^ \s* use \s+ Latin6 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list !~ m/\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Latin6::ord'     if m/\A ord \z/oxms;
                    $function_ord_    = 'Latin6::ord_'    if m/\A ord \z/oxms;
                    $function_reverse = 'Latin6::reverse' if m/\A reverse \z/oxms;
                }
            }
        }
    }
...
# functions of package Elatin6
    elsif (m{\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  }oxgc) { $slash = 'm//'; return $1;                    }
    elsif (m{\G \b bytes::substr \b (?! \s* => )                }oxgc) { $slash = 'm//'; return 'substr';              }
    elsif (m{\G \b chop \b          (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Elatin6::chop';         }
    elsif (m{\G \b bytes::index \b  (?! \s* => )                }oxgc) { $slash = 'm//'; return 'index';               }
    elsif (m{\G \b Latin6::index \b   (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Latin6::index';         }
    elsif (m{\G \b index \b         (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Elatin6::index';        }
    elsif (m{\G \b bytes::rindex \b (?! \s* => )                }oxgc) { $slash = 'm//'; return 'rindex';              }
    elsif (m{\G \b Latin6::rindex \b  (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Latin6::rindex';        }
    elsif (m{\G \b rindex \b        (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Elatin6::rindex';       }
    elsif (m{\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin6::lc';           }
    elsif (m{\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin6::lcfirst';      }
    elsif (m{\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin6::uc';           }
    elsif (m{\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin6::ucfirst';      }
    elsif (m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin6::fc';           }
    ...
    elsif (m{\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin6::chr';               }
    ...
    elsif (m{\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin6::glob';              }
    elsif (m{\G \b lc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin6::lc_';               }
    elsif (m{\G \b lcfirst \b    (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin6::lcfirst_';          }
    elsif (m{\G \b uc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin6::uc_';               }
    elsif (m{\G \b ucfirst \b    (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin6::ucfirst_';          }
    elsif (m{\G \b fc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin6::fc_';               }
    ...
    elsif (m{\G \b chr \b           (?! \s* => )                      }oxgc) { $slash = 'm//'; return 'Elatin6::chr_';              }
    elsif (m{\G \b bytes::ord \b    (?! \s* => )                      }oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (m{\G \b ord \b           (?! \s* => )                      }oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (m{\G \b glob \b          (?! \s* => )                      }oxgc) { $slash = 'm//'; return 'Elatin6::glob_';             }
...
# functions of package Elatin6
        ...
        elsif ($string =~ m{\G \b chop \b                                      }oxgc) { $e_string .= 'Elatin6::chop';    $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::index \b                              }oxgc) { $e_string .= 'index';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b Latin6::index \b                               }oxgc) { $e_string .= 'Latin6::index';    $slash = 'm//'; }
        elsif ($string =~ m{\G \b index \b                                     }oxgc) { $e_string .= 'Elatin6::index';   $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::rindex \b                             }oxgc) { $e_string .= 'rindex';         $slash = 'm//'; }
        elsif ($string =~ m{\G \b Latin6::rindex \b                              }oxgc) { $e_string .= 'Latin6::rindex';   $slash = 'm//'; }
        elsif ($string =~ m{\G \b rindex \b                                    }oxgc) { $e_string .= 'Elatin6::rindex';  $slash = 'm//'; }
        elsif ($string =~ m{\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin6::lc';      $slash = 'm//'; }
        elsif ($string =~ m{\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin6::lcfirst'; $slash = 'm//'; }
        elsif ($string =~ m{\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin6::uc';      $slash = 'm//'; }
        elsif ($string =~ m{\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin6::ucfirst'; $slash = 'm//'; }
        elsif ($string =~ m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin6::fc';      $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin6::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin6::glob';              $slash = 'm//'; }
        elsif ($string =~ m{\G \b lc \b                                              }oxgc) { $e_string .= 'Elatin6::lc_';               $slash = 'm//'; }
        elsif ($string =~ m{\G \b lcfirst \b                                         }oxgc) { $e_string .= 'Elatin6::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b uc \b                                              }oxgc) { $e_string .= 'Elatin6::uc_';               $slash = 'm//'; }
        elsif ($string =~ m{\G \b ucfirst \b                                         }oxgc) { $e_string .= 'Elatin6::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b fc \b                                              }oxgc) { $e_string .= 'Elatin6::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b chr \b                                             }oxgc) { $e_string .= 'Elatin6::chr_';              $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::ord \b                                      }oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ m{\G \b ord \b                                             }oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ m{\G \b glob \b                                            }oxgc) { $e_string .= 'Elatin6::glob_';             $slash = 'm//'; }
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Elatin6::uc($char[$i]) ne Elatin6::fc($char[$i]))) {
...
Latin6 - Source code filter to escape Latin-6

=head1 SYNOPSIS

  use Latin6;
  use Latin6 version;         --- require version
...
=head1 Calling 'Elatin6::ignorecase()' (Latin6.pm provides)

Latin6.pm applies calling 'Elatin6::ignorecase()' instead of /i modifier.
...
=head1 Un-Escaping \ Of \N, \p, \P and \X (Latin6.pm provides)

Latin6.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Latin6.pm and Elatin6.pm provide)
...
=head1 Un-Escaping bytes::* Functions (Latin6.pm provides)

Latin6.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Elatin6.pm provides)

Elatin6.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Latin6' }" at head.
Store the standard module modified for Latin6 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Latin6 and change
'use utf8;' to 'use Latin6;' in its. You need help yourself for now.
...
Contrasting is Encode module and describing "use Latin6;" on this software,
